...
As a result, provide a link to github ru.javawebinar.topjava.repository. It should contain the code, README.md with API documentation and couple curl commands to test it (**better - link to Swagger**).
...
- 6.1: Используйте Spring Data JPA (без лишней делегации). Методы ru.javawebinar.topjava.repository можно вызывать напрямую из сервиса или из контроллера.
...
```
if(updateCondition)
   ru.javawebinar.topjava.repository.delete(entity)
}
ru.javawebinar.topjava.repository.save(entity)
```
